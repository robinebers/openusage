...
final class ProviderHeaderNameTests: XCTestCase {
    ...
    func testMultiAccountHeaderDropsTheAgent() {
        // The provider mark already names the agent, so the header spends that width on the account.
        let provider = make(id: "claude@3ac4b63e", name: "Claude: Acme (jane@example.com)")

        XCTAssertEqual(provider.headerName(), "Acme (jane@example.com)")
    }

    func testSingleAccountHeaderIsUnchanged() {
        XCTAssertEqual(make(id: "cursor", name: "Cursor").headerName(), "Cursor")
        XCTAssertEqual(make(id: "claude", name: "Claude").headerName(), "Claude")
    }
    ...
    func testHeaderMasksTheEmailLeftAfterDroppingTheAgent() {
        ...
        XCTAssertEqual(provider.headerName(hidingEmails: true), "Acme (j•••e@•••••.•••)")
    }
