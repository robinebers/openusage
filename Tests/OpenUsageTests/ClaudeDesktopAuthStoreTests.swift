...
final class ClaudeDesktopAuthStoreTests: XCTestCase {
    ...
    // MARK: - Multi-account (Desktop organizations as picker accounts)

    func testOrganizationInventoryListsEveryOrgAndTheActiveOne() throws {
        let fixture = try makeFixture(
            activeOrganization: organization,
            v2: [
                cacheKey(organization: organization): tokenEntry("desktop-token", expiresIn: 3_600),
        ...
        let inventory = try XCTUnwrap(fixture.store.organizationInventory(allowInteraction: false))

        XCTAssertEqual(inventory.organizations, [organization, otherOrganization].sorted())
        XCTAssertEqual(inventory.active, organization)
    }

    func testLoadForASpecificOrganizationSkipsTheActiveOrgCookie() throws {
        let fixture = try makeFixture(
            activeOrganization: organization,
            v2: [
                cacheKey(organization: organization): tokenEntry("desktop-token", expiresIn: 3_600),
    ...
    func testDiscoveryTurnsNonActiveDesktopOrganizationsIntoAccounts() throws {
        let fixture = try makeFixture(
            activeOrganization: organization,
            v2: [
                cacheKey(organization: organization): tokenEntry("desktop-token", expiresIn: 3_600),
        ...
        let extras = ClaudeAccountDiscovery(authStore: authStore, keychain: ServiceKeychain())
            .discoverExtraAccounts()

        XCTAssertEqual(extras.map(\.desktopOrganization), [otherOrganization],
                       "the active organization feeds the default card; every other org is an extra account")
    }

    func testScopedDesktopAccountLoadsOnlyItsOrganizationToken() throws {
        let fixture = try makeFixture(
            activeOrganization: organization,
            v2: [
                cacheKey(organization: organization): tokenEntry("desktop-token", expiresIn: 3_600),
        ...
        let scoped = ClaudeAuthStore(
            ...
            files: FakeFiles(),
            keychain: ServiceKeychain(values: ["Claude Code-credentials": #"{"claudeAiOauth":{"accessToken":"cli"}}"#]),
            desktop: fixture.store,
            account: ClaudeAccountScope(desktopOrganization: otherOrganization)
        )

        let tokens = scoped.loadCredentialCandidates().compactMap(\.oauth.accessToken)
        XCTAssertEqual(tokens, ["other-token"], "a Desktop-org scope ignores the CLI keychain and the env token")
    }
