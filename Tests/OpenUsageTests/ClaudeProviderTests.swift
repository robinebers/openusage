...
final class ClaudeProviderTests: XCTestCase {
    ...
    func testLiveClaudeUsageReportsResetFields() async throws {
        ...
        guard let state = try store.loadCredentialCandidates().first else {
    ...
    func testDesktopAppOnlyLoginExplainsCLILoginInsteadOfNotLoggedIn() async {
        ...
        // A present credential file with a blank token is malformed, not absent. It must surface the
        // repairable credential-data error rather than being relabeled as a desktop-only or missing login.
        let corruptCLI = makeProvider(files: FakeFiles([
            "~/.claude/.credentials.json": #"{"claudeAiOauth":{"accessToken":"   "}}"#,
            "~/Library/Application Support/Claude/claude-code": ""
        ]))
        let corruptSnapshot = await corruptCLI.refresh()
        XCTAssertEqual(badge(corruptSnapshot.lines, "Error"), ClaudeAuthError.invalidCredentialData.localizedDescription)
        XCTAssertEqual(corruptSnapshot.errorCategory, .authInvalid)
    }
