...
final class ClaudeAuthStoreTests: XCTestCase {
    ...
    func testCredentialDiagnosticsLabelIsTokenFreeWithSourceRefreshAndExpiredFlags() {
        ...
        let unknownExpiry = ClaudeCredentialState(
            oauth: ClaudeOAuth(accessToken: "a", refreshToken: " \n ", expiresAt: nil),
    ...
    func testMalformedCustomOAuthURLThrowsInsteadOfCrashing() {
        // Foundation accepts several strings that are not usable/safe OAuth endpoints. Reject every
        // non-HTTP(S), relative, hostless, credential-bearing, query, and fragment form without echoing
        // potentially sensitive boundary input into the error.
        let invalidValues = [
            "http://exa mple.com",
            "not-a-url",
            "localhost:8000",
            ...
            "https://example.com?token=top-secret",
            "https://example.com#private-fragment",
        ]

        for value in invalidValues {
            let store = ClaudeAuthStore(
                environment: FakeEnvironment(["CLAUDE_CODE_CUSTOM_OAUTH_URL": value]),
                files: FakeFiles(),
                keychain: FakeKeychain()
            )

            XCTAssertThrowsError(try store.oauthConfig(), value) { error in
                ...
                XCTAssertEqual(
                    ...
                    "Invalid Claude OAuth URL configuration. Check CLAUDE_CODE_CUSTOM_OAUTH_URL / CLAUDE_LOCAL_OAUTH_API_BASE."
                )
                XCTAssertFalse(error.localizedDescription.contains(value), value)
            }

            // The forgiving credential-load path only needs the file suffix, so malformed URLs must
            // not break keychain candidate resolution.
            XCTAssertEqual(store.keychainServiceCandidates(), ["Claude Code-custom-oauth-credentials"])
        }
    }

    func testValidCustomOAuthURLAllowsHTTPPortAndBasePath() throws {
        let store = ClaudeAuthStore(
            environment: FakeEnvironment([
                "CLAUDE_CODE_CUSTOM_OAUTH_URL": "http://localhost:8000/oauth-base/"
            ]),
            files: FakeFiles(),
            keychain: FakeKeychain()
        )

        let config = try store.oauthConfig()

        XCTAssertEqual(config.usageURL.absoluteString, "http://localhost:8000/oauth-base/api/oauth/usage")
        XCTAssertEqual(config.refreshURL.absoluteString, "http://localhost:8000/oauth-base/v1/oauth/token")
    }
}
