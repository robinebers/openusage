...
**Metrics without a limit** (daily spend, balances) show as a single line like `$4.08 spent` or `1.2M tokens`. The Today / Yesterday / Last 30 Days / Month to Date rows combine cost and tokens (`$4.08 · 1.2M tokens`) and can be turned on or off in Customize. **Month to Date** totals from the first day of the current calendar month through today and is also controlled by the off-by-default **Show Month-to-Date Spending** switch in Settings. A day with no usage reads "No data" rather than a misleading `$0.00 · 0 tokens` — the same as when the source can't be loaded at all. Big numbers are abbreviated to keep rows tidy (`$2.06K`, `1.5B`) — hover the value to see the exact figures and source note, such as a local estimate.
