...
- A blue bar carries nothing extra by default. With **Always Show Pacing** on (Settings), it also shows an even-pace tick on the bar and a quiet projection next to the metric name, matching the active Used/Left mode.
...
- **Hover the bar**, the spare note, or the flame for an explicitly labeled pace projection. Blue and yellow projections match the active mode (`Projected ~65% used at reset` or `Projected ~35% left at reset`); red shows the expected overage (`Projected ~12% over limit at reset`, or `Projected ~100% used at reset` when landing right at the limit). Once spent it reads `Limit reached`.
