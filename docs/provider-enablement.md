...
An extra Claude account (see [Claude § Multiple accounts](providers/claude.md#multiple-accounts)) rides the exact same path: each one is a provider OpenUsage has never seen, so it turns on the first time it's detected with credentials, and is yours to manage from then on.
