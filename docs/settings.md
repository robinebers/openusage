...
All three alerts default off. The first time you turn one on, OpenUsage asks for notification permission and waits for the macOS prompt to finish before updating the warning and action below the toggles. If you decline (or turn notifications off for OpenUsage in System Settings later), a warning mark appears on the Notifications header and an "Open System Settings" button shows under the toggles so you can re-enable them; once macOS has recorded a denial, OpenUsage opens System Settings instead of trying to prompt again. A notification's title is the alert name, its subtitle names the provider and metric, and its body is the plain-language verdict. Tapping an alert opens the popover on the dashboard.
