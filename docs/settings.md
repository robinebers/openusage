...
| Enable Mini Cards | Off / On | Off by default. On lets you collapse any provider on the dashboard to a single header line. Turning it on doesn't collapse anything by itself; it adds the affordance. See [Dashboard](dashboard.md#mini-cards). |
| Mini Card Style | Single Row / Detailed | Only shown when Mini Cards are on. **Single Row** keeps everything on the header line: the meters ride the trailing edge as a small pill, and the plan badge steps aside to make room, so a long provider name may truncate. **Detailed** gives the meters their own line under the header, each labeled with its metric name, and keeps the full name and plan badge. |
| Hide Agent Name for Multi-Account Card Headers | Off / On | Off by default. When you have several accounts of one agent signed in, every card header repeats the agent name (`Claude: Acme (jane@example.com)`). On drops that prefix so the header reads `Acme (jane@example.com)` and spends the width on the account instead. The provider icon still says which agent it is. Providers with a single account carry no prefix and don't change. |
...
| Hide Emails | On / Off | Off (default). On masks account email addresses everywhere OpenUsage shows them: card headers, Customize, the Total Spend legend, menus, notifications, and screenshots you copy or share. Each address keeps its first and last characters before the `@`, and every domain becomes the same placeholder, so `jane@example.com` reads `j•••e@•••••.•••`. The local API and the `openusage` command keep real addresses, since tools that read them match on identity. |
