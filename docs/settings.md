...
Your settings carry across updates — layout, stars, preferences, and the menu-bar shortcut all stay put. When an update changes how a setting is stored, the app upgrades it in place on launch, stepping through any in-between versions if you skipped a few. Saved layouts are also cleaned up in place if a build left duplicate entries; the first surviving copy keeps its position. Entries an older build doesn't recognize stay preserved for a newer version, even if you customize or reset the known parts of the layout while using the older build; the older build simply hides those entries. An update never resets these settings. (Earlier betas wiped all settings on every update; that no longer happens.)
