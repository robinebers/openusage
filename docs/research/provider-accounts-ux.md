...
- **Keychain ACLs can freeze launch.** Attribute-only enumeration is prompt-free, but any *secret read* can block behind a hidden ACL dialog; `LAContext`/`kSecUseAuthenticationUIFail` do **not** stop the file-based login keychain from blocking. #987 had to move discovery off the launch path and gate interactive reads behind manual refresh.
...
**D. Keychain enumeration as discovery (#987's Claude path).** *Rejected as a discovery channel.* Ground truth on one real machine: 178 `Claude Code-credentials*` items, one real; suffix hashes can't be mapped back to dirs (one-way); junk heuristics (mdat−cdat>24h) are guesswork; secret reads can freeze launch behind ACL dialogs. Keychain stays as a **computed lookup**: given a dir we already trust, derive its item name (`hashSuffix`) and read that one item — no enumeration, no heuristics, at most one predictable prompt on a user action.
...
Nothing in tiers 1–2 reads a keychain secret or can prompt; tier 2's Claude keychain check is attributes-only on a *computed* item name. Secret reads happen on first refresh of an added instance — at most one predictable macOS prompt per account, caused by a visible user action.
...
- Enablement: instance ids flow through `NewProviderSeeder` (never-seen + credentials → enabled once; a later manual off is never overridden). `hasLocalCredentials()` for scoped instances is footprint-only (file/keychain-attributes/desktop status) so seeding can never trigger a keychain dialog; the one-time macOS prompt for an extra keychain-backed account happens on its **first refresh** — the trade #987 already accepted and documented.
- Exclusions kept simple in P1: iCloud history syncs default instances only (instance ids filtered); the Codex reset-claim service stays bound to the default Codex; pi fold-in stays on default cards; telemetry reports instance ids as-is (hash suffix is pseudonymous).
