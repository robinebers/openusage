...
If you run more than one Claude account on this Mac — say a work login in its own `CLAUDE_CONFIG_DIR` folder, or a Cowork/Claude Desktop login that differs from your CLI login — OpenUsage finds each one at launch and shows it as its **own card** ("Claude 1", "Claude 2"), with its own meters, its own spend tiles from its own logs, and its own toggle in Customize. Nothing is shared between cards: each extra account reads exactly its one login and can never see another account's credentials.

Discovery is read-only and quiet: it looks for folders that carry a real Claude login *and* can name their account, and it never opens a keychain secret while scanning — so it can't trigger permission dialogs at launch. Folders and Cowork sandboxes that authenticate the **same** account are folded into one card, but OpenUsage still combines the local logs from every matching source so none of that account's spend disappears. The first time an extra keychain-backed account actually refreshes, macOS may show its usual one-time permission dialog for that account's keychain item — choose **Always Allow** once and it won't ask again.

Accounts are organization-aware: a Team org and a personal Max org under the same email are different usage pools, so they get different cards. On a multi-account install, the default card's Claude Desktop fallback is pinned to that card's organization; when it can't be pinned safely, OpenUsage leaves the fallback off instead of borrowing another card's Desktop login.

If you use [claude-swap](https://github.com/realiti4/claude-swap) (`cswap`), each **parked** slot shows as its own card too. OpenUsage reads the slot's `.enc` vault backup first and falls back to the matching Keychain item only when that backup is missing or unreadable; both are read-only and never refreshed or modified. A stale parked token shows a re-login notice until cswap next touches it. The active slot follows the account in Claude's live config rather than trusting cswap's potentially stale sequence record, so it remains the default Claude card without appearing twice.

Account cards are assembled at launch. After switching the active cswap account, restart OpenUsage so it can rebuild those cards and their spend ownership; until then, refresh is refused rather than publishing the new account under the previous card's identity.

On a cswap machine all accounts write into the **same** session logs, so the spend tiles are split by time. OpenUsage reads the current switch log plus cswap's three retained rotated logs, oldest first, and attributes each entry to whichever account was active then. Per-account spend remains an estimate at switch boundaries because a session already running when you swap can keep billing the old account for a few minutes. Once rotation may have discarded older switch history, usage before the oldest retained record stays on the default card instead of being guessed; without any readable switch history, all shared-home spend stays there.
...
- **"Claude's active account changed"** — a switch tool changed the default login after OpenUsage launched. Restart OpenUsage so account cards and local history are rebound before refreshing.
