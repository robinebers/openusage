...
Once authenticated via gh CLI, the plugin caches the token in the UsageTray keychain for faster access on subsequent probes. The plugin also reads the legacy `OpenUsage-copilot` service for migration.
