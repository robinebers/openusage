...
Sign in with the Codex CLI (`codex`); OpenUsage reads the same auth files (`$CODEX_HOME` respected) with a keychain fallback. Tokens refresh automatically and rotate back into the auth file.

## Multiple accounts

Keep each login in its own Codex home—for example, `~/.codex-personal` and `~/.codex-work`—and sign in with the CLI once in each home. At launch, OpenUsage scans file-backed homes named `~/.codex*` and `~/.config/codex*`. Every OAuth login that identifies its ChatGPT account becomes a separate card with its own limits, plan, reset credits, and local spend from that home's rollout logs.

Two homes signed into the same ChatGPT account fold into one card instead of duplicating it. Extra cards use stable ids such as `codex@ab12cd34`, can be renamed from the card's right-click menu or Customize, and keep their layout and history if the home temporarily disappears. Restart OpenUsage after adding or removing a home; discovery runs at launch.

Keyring-only extra homes are not discovered yet because their credential cannot be safely bound to a home without reading a Keychain secret during launch. The existing bare Codex card still supports its normal keychain fallback. Extra account cards require an `auth.json` containing an OAuth login with `tokens.account_id` (or the equivalent account claim in `id_token`).

In the [CLI](../cli.md) and [local API](../local-http-api.md), requesting `codex` returns every Codex account card; request a full `codex@…` id for one account.

## The spend tiles

Today / Yesterday / Last 30 Days are computed **locally**: OpenUsage reads the Codex CLI's session rollouts under `~/.codex/sessions/` and `archived_sessions/` (or `$CODEX_HOME`) itself — no external tools needed. With multiple accounts, each card reads only its own home's rollouts. Symlinks are followed, so a Codex home linked into a synced location (say, a Dropbox folder) is read all the same. Codex usage from the [pi](https://github.com/earendil-works/pi) coding agent counts too: OpenUsage reads pi's session logs under `~/.pi/agent/sessions/` (or `$PI_CODING_AGENT_SESSION_DIR`) and folds any Codex usage there into the bare Codex card's tiles and trend; pi logs do not identify the ChatGPT account, so they are not assigned to extra account cards. pi records its own per-message cost, so those dollars come straight from pi rather than being re-estimated. Days are grouped in your Mac's local time zone, so they line up with your own calendar. Each period is one tile showing cost and tokens together (`$4.08 · 1.2M tokens`); a day with no usage reads **No data** rather than a misleading `$0.00 · 0 tokens` — the same as every other spend-tracking provider. The live Session and Weekly meters are unaffected. The dollars are estimated from token counts at API rates (that's the ⓘ) using the shared [model pricing](../pricing.md); sessions that ran on the fast/priority service tier — as recorded in each session's own log — use the fast rates for exactly those turns. Older logs without tier metadata, and everything else, price at standard rates; the current `config.toml` setting is not consulted, so flipping the tier never reprices past days. Auto-review usage keeps its `codex-auto-review` name in the model breakdown, while its cost uses the dated model fallback available for that event. The token counts themselves are measured. Subagent and forked sessions copy their parent session's token history into their own log; OpenUsage recognizes those copies and counts each token once, no matter how many subagents a session spawns. No log data leaves your Mac.
...
- **A second account does not appear** — keep it in a sibling home named `~/.codex*` or `~/.config/codex*`, make sure that home has a file-backed OAuth `auth.json`, then restart OpenUsage. Keyring-only extra homes are not discovered yet.
