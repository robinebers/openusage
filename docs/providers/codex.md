...
**Customize → Codex → Cost Estimates → Fallback Model** optionally estimates usage that has no known price. The default is **None**. Choose a public model to use its rates for those estimates; known model prices and recorded costs remain unchanged. Switching the choice recalculates local history without changing the model Codex runs. See [model pricing](../pricing.md) for details.
