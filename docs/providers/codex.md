...
- **OpenCode usage is missing** — OpenCode must currently have an `openai` OAuth credential in
  `auth.json` or, on OpenCode 2, in the `credential` table. An OpenAI API key is deliberately
  excluded from Codex subscription totals.
