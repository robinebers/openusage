...
- **Spend tiles show "No data"** — OpenUsage found no qualifying Codex usage in Codex, pi, OpenCode, or Capy from the last 30 days. If your Codex home lives somewhere custom, set `CODEX_HOME` so both the Codex CLI and OpenUsage look in the same place.
- **Capy usage is missing** — sign in to the Capy desktop app and check that Capy's Codex connection uses the same ChatGPT account as this card. Then refresh the Codex card manually and choose **Always Allow** if macOS asks about "Capy Safe Storage". Background refreshes never show that prompt, so until it's allowed they skip Capy (logged under `plugin:capy`).
