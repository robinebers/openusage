...
When a rolling window still has (nearly) a full period left before reset (`reset_after_seconds` ≈ `limit_window_seconds`, with a small grace for fetch latency — Codex stamps `reset_at` server-side, so it always arrives a moment stale), OpenUsage treats it as a fresh window: a `used_percent` of 0–1% is normalized to unused (Codex’s whole-percent floor), and burn-rate pacing waits until the window has materially started before projecting. While the current Session window has no usage, the Session row reads **Not started** on the trailing label, and the hover explains that the session begins after your first message.
