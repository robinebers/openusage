...
- Account cards from the same provider share menu-bar stars. Star Codex Weekly once and every enabled Codex account contributes its Weekly value. When one metric is starred, those account percentages stack under one provider icon in the same top-to-bottom order as their cards; dragging the cards immediately updates the stack order.
