...
describe("codex plugin", () => {
  ...
  it("uses CODEX_HOME auth path when env var is set", async () => {
    ...
    ctx.host.env.get.mockImplementation((name) =>
      name === "CODEX_HOME" ? "/tmp/codex-home" : null,
    );
  ...
  it("does not fall back when CODEX_HOME is set but missing auth file", async () => {
    const ctx = makeCtx();
    ctx.host.env.get.mockImplementation((name) =>
      name === "CODEX_HOME" ? "/tmp/missing-codex-home" : null,
    );
    ctx.host.fs.writeText(
      "~/.config/codex/auth.json",
      JSON.stringify({
  ...
  it("refreshes token and formats usage", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      if (String(opts.url).includes("oauth/token")) {
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "new" }),
        };
      }
      return {
        status: 200,
  ...
  it("refreshes keychain auth and writes back to keychain", async () => {
    ...
    const [service, payload] =
      ctx.host.keychain.writeGenericPassword.mock.calls[0];
  ...
  it("omits token lines when ccusage reports no_runner", async () => {
    ...
    expect(
      result.lines.find((l) => l.label === "Last 30 Days"),
    ).toBeUndefined();
  ...
  it("queries ccusage on each probe", async () => {
    ...
    ctx.host.ccusage.query.mockReturnValue({
      status: "ok",
      data: {
        daily: [{ date: "2026-02-01", totalTokens: 100, totalCost: 0.5 }],
      },
    });
  ...
  it("shows empty Yesterday state when yesterday's totals are zero (regression)", async () => {
    ...
    ctx.host.ccusage.query.mockReturnValue({
      status: "ok",
      data: {
        daily: [{ date: yesterdayKey, totalTokens: 0, costUSD: 0 }],
      },
    });
  ...
  it("shows empty Today when history exists but today is missing (regression)", async () => {
    ...
    ctx.host.ccusage.query.mockReturnValue({
      status: "ok",
      data: {
        daily: [{ date: "Feb 01, 2026", totalTokens: 300, costUSD: 1.0 }],
      },
    });
  ...
  it("adds Yesterday line from codex ccusage format", async () => {
    ...
    ctx.host.ccusage.query.mockReturnValue({
      status: "ok",
      data: {
        daily: [{ date: yesterdayKey, totalTokens: 220, costUSD: 1.1 }],
      },
    });
  ...
  it("matches UTC timestamp day keys at month boundary (regression)", async () => {
    ...
    try {
      ...
      ctx.host.ccusage.query.mockReturnValue({
        ...
        data: {
          daily: [
            { date: "2026-03-01T12:00:00Z", totalTokens: 10, costUSD: 0.1 },
          ],
        },
      });
  ...
  it("throws on usage request failure after refresh", async () => {
    ...
    expect(() => plugin.probe(ctx)).toThrow(
      "Usage request failed after refresh",
    );
  });
  ...
  it("surfaces additional_rate_limits as Spark lines", async () => {
    ...
    expect(sparkWeekly.resetsAt).toBe(
      new Date((nowSec + 86400) * 1000).toISOString(),
    );
  ...
  it("handles additional_rate_limits with missing fields and fallback labels", async () => {
    ...
    expect(
      result.lines.find((l) => l.label === "Model Weekly"),
    ).toBeUndefined();
  ...
  it("falls back to existing token when refresh cannot produce new access token", async () => {
    ...
    const runCase = async (refreshResp) => {
      ...
      const result = plugin.probe(ctx);
      expect(
        result.lines.find((line) => line.label === "Session"),
      ).toBeTruthy();
    };
  ...
  describe("rate limit reset status", () => {
    ...
    it("shows Yes when rate limit has reset", async () => {
      ...
      const plugin = await loadPlugin();
      const result = plugin.probe(ctx);
      const line = result.lines.find((l) => l.label === "Reset today?");
    ...
    it("skips rate limit badge when API request fails", async () => {
      ...
      const result = plugin.probe(ctx);
      expect(
        result.lines.find((l) => l.label === "Reset today?"),
      ).toBeUndefined();
    });
    ...
    it("shows subtitle with relative time from updatedAt", async () => {
      ...
      try {
        ...
        const line = result.lines.find((l) => l.label === "Reset today?");
        expect(line).toBeTruthy();
        expect(line.subtitle).toBe("· 10m ago");
      } finally {
        vi.useRealTimers();
      }
    });
    ...
    it("shows hours in subtitle when updatedAt is hours ago", async () => {
      ...
      try {
        ...
        const line = result.lines.find((l) => l.label === "Reset today?");
        expect(line).toBeTruthy();
        expect(line.subtitle).toBe("· 2h ago");
      } finally {
        vi.useRealTimers();
      }
    });
