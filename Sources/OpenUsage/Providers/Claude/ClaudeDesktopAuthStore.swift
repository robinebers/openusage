...
struct ClaudeDesktopAuthStore: Sendable {
    ...
    /// Loads a Desktop credential — the active organization's (the cookie lookup) by default, or a
    /// SPECIFIC organization's when `organization` is given (an extra-account scope pinned to that
    /// login; the cookie is skipped entirely).
    func load(allowInteraction: Bool, organization: String? = nil) -> ClaudeDesktopCredentialResult {
        ...
        do {
            ...
            guard let targetOrg = try organization ?? loadActiveOrganization(key: key),
    ...
    /// Every organization with a usable-scope token entry in Desktop's cache, plus the active one —
    /// account discovery turns each NON-active organization into a picker account (the active org
    /// already feeds the default card's Desktop fallback). Returns `nil` when the cache can't be
    /// read without a prompt (`allowInteraction: false` before the user has granted Safe Storage
    /// access) — discovery then finds no Desktop accounts until the one-time grant has happened.
    func organizationInventory(allowInteraction: Bool) -> (organizations: [String], active: String?)? {
        guard hasCredentialMaterial() else { return nil }
        do {
            guard let key = try safeStorageKey(allowInteraction: allowInteraction),
                  let caches = try loadCaches(key: key)
            else {
                return nil
            }
            var organizations = Set<String>()
            for cache in [caches.v2, caches.v1] {
                guard let cache else { continue }
                for (cacheKey, rawEntry) in cache {
                    guard let parsed = Self.parseCacheKey(cacheKey),
                          parsed.apiHost == Self.apiHost,
                          parsed.scopes.contains(Self.usageScope),
                          !(rawEntry is NSNull)
                    else { continue }
                    organizations.insert(parsed.organization)
                }
            }
            let active = try? loadActiveOrganization(key: key)
            return (organizations.sorted(), active ?? nil)
        } catch {
            AppLog.warn(LogTag.auth("claude"), "Claude Desktop organization inventory unavailable: \(error.localizedDescription)")
            return nil
        }
    }
