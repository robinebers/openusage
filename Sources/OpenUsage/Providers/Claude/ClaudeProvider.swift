...
final class ClaudeProvider: ProviderRuntime {
    ...
    /// with an `@`-suffixed id and an ordinal display name ("Claude 2"); everything else about the
    ...
    func hasLocalCredentials() async -> Bool {
        // Scoped instances answer from footprints only (file existence / keychain attributes /
        // Desktop status) so the every-launch seeding probe can never raise a keychain dialog for an
        // account the user hasn't granted yet. The secret read happens on the first refresh.
