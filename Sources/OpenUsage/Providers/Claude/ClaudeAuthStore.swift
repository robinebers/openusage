...
struct ClaudeAuthStore: Sendable {
    ...
    func keychainServiceCandidates() -> [String] {
        // An account-scoped store reads only its own pinned keychain service (or none, for a
        // file-only account) — never the default service or the env-derived hash.
        if let account {
            return account.keychainService.map { [$0] } ?? []
        }
        if let configDir = claudeHomeOverride() {
            return ["\(baseKeychainService())-\(hashSuffix(configDir))", baseKeychainService()]
        }
        return [baseKeychainService()]
    }

    /// The bare (default-account) keychain service — `Claude Code<suffix>-credentials` with no
    /// config-dir hash. Multi-account discovery enumerates keychain items under this prefix: every
    /// hash-suffixed sibling is another login. Only needs the endpoint suffix, which never fails —
    /// kept off the throwing URL path so credential loading stays forgiving even when a custom OAuth
    /// URL is malformed.
    ...
    /// The credentials file to read/write, or `nil` when this store has no file source (a
    /// keychain-only scoped account). The default instance always resolves a path (env override or
    /// `~/.claude`).
