...
struct WidgetRegistry: Sendable {
    ...
    /// canonical registry order — except provider instances (`claude@ab12cd34`), which slot in right
    /// after their base provider's group so a newly discovered account appears next to its siblings
    ...
    func orderedProviderIDs(savedOrder: [String]) -> [String] {
        ...
        for id in defaults where !savedIDs.contains(id) {
            let base = ProviderInstanceID.base(of: id)
            if ProviderInstanceID.isInstance(id),
               let anchor = result.lastIndex(where: { ProviderInstanceID.base(of: $0) == base }) {
