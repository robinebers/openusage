extension LayoutStore {
    ...
    /// Pinned metrics grouped by provider card, in the user's Customize order (provider order, then
    /// each provider's metric order). Stars are per card, so each Codex account contributes only the
    /// metrics starred on it. A temporarily disabled card is excluded from the rendered groups but
    /// keeps its pins. Drives the menu-bar strip.
    var pinnedGroups: [ProviderMetrics] {
        orderedProviders().compactMap { provider -> ProviderMetrics? in
            guard isProviderEnabled(provider.id) else { return nil }
            // Keep the strip order matching Customize: always-shown pins first, then expanded ones.
            let metrics = orderedSupportedMetrics(for: provider.id).filter {
