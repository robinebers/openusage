extension LayoutStore {
    ...
    func reorderProvider(dragged: String, target: String) -> Bool {
        recordingUndoStep {
            ...
            // Reorder only the visible slots in the raw persisted sequence. Unknown ids may be account
            // instances suppressed from this launch's registry, and disabled providers are hidden from
            // `customizeGroups`; both keep their exact positions while the visible ids move around them.
