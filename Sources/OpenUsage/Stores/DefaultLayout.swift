...
enum DefaultLayout {
    ...
    /// card — including its default menu-bar stars, which are per card. `migrationBaselineMetricIDs`
    /// is deliberately NOT translated: account-card ids must always read as never-offered so their
