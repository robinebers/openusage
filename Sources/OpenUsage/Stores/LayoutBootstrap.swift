...
enum LayoutBootstrap {
    static func load(
        ...
        // Keep widgets whose provider is temporarily absent from this launch's registry. They remain
        // invisible because rendering resolves through the live registry, but carrying the tombstones
        // through unrelated layout writes lets an account instance recover its enabled state when it
        // returns.
        ...
        // Unknown saved ids are retained as invisible tombstones for temporarily suppressed instances.
        let pinnedMetricIDs: Set<String>
        ...
        if let savedOnEnable {
            ...
            // temporarily suppressed provider instance and must survive until its descriptor returns.
    ...
    private static func seedNewDefaultMetrics(
        ...
        if let saved = persistence.loadSeededDefaults() {
            // Keep markers for metrics whose provider is temporarily absent from this launch's registry
            // (for example, an account instance suppressed while it is the default login). Pruning them
            // would make a default metric the user disabled look newly introduced when the instance
            // returns, so startup would turn it back on. Permanently removed metric ids are harmless
            // tombstones and can stay here.
...
enum LayoutOrdering {
    ...
    static func normalizedMetricOrder(
        ...
        // Start with every saved provider so a temporarily absent account instance keeps its ordering
        // entry. For providers present now, deduplicate the saved sequence (including unknown metric
        // tombstones) and append newly introduced live metrics; `LayoutStore.metricOrder(for:)` filters
        // this persisted superset through the live registry before rendering.
        var fallback = saved
        for provider in registry.providers {
