...
struct Provider: Identifiable, Hashable {
    ...
    /// What a card header calls this provider. "Hide Agent Name for Multi-Account Card Headers" drops
    /// the agent prefix every one of a multi-account provider's cards repeats; "Hide Emails" then
    /// masks any address left in the name. Providers with neither are unaffected.
    func headerName(hidingAgentName: Bool, hidingEmails: Bool = false) -> String {
        let name = (hidingAgentName ? accountLabel : nil) ?? displayName
