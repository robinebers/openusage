...
struct WidgetData: Hashable {
    ...
    /// Period-scoped model spend/tokens for the Today / Yesterday / Last 30 Days / Month to Date hover popover. Nil for
    ...
    /// True only for the Today / Yesterday / Last 30 Days / Month to Date spend tiles, where the row's values accumulate
    ...
    var unboundedValueTooltip: String? {
        ...
        // The "no usage" note only fits a spend period (Today / Yesterday / Last 30 Days / Month to Date), where a zero
...
extension WidgetData {
    ...
    /// (`!isBounded`); a run of them (Today / Yesterday / Last 30 Days / Month to Date) pulls up into one cluster. The
