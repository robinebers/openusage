...
struct ProviderSectionHeader: View {
    ...
    /// The chevron replaces the mark while the pointer is on something a click would act on. On an
    /// open card that is the mark or the name. A collapsed card is small enough to be one target, so
    /// anywhere on it counts, its meters included.
    private var showsChevron: Bool {
        miniCard != nil && (isMarkHovered || isNameHovered || (isMinimized && isHovered))
    }
    ...
    var body: some View {
        ...
        // A collapsed card expands from a click anywhere on it, its meters included, not only its mark
        // or name. This has to be a button: the dashboard gives every header a high-priority drag
        // gesture for reordering, which swallows a plain tap gesture but still lets buttons through.
        // It sits over the card only while collapsed, so the layout underneath never changes and the
        // open card's own mark and name buttons are untouched. Being on top, it takes the click once;
        // the mark and name buttons beneath it never also fire.
        .overlay {
            if let miniCard, miniCard.isMinimized {
                Button {
                    toggle(miniCard)
                } label: {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(toggleAccessibilityLabel(minimized: true))
            }
        }
        // Tracked outside the overlay: a button on top would otherwise absorb the pointer, and hovering
        // a collapsed card's meters would stop showing the chevron.
        .onHover { isHovered = $0 }
    }
