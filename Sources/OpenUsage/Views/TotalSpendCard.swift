...
struct TotalSpendCard: View {
    ...
    private var total: TotalSpend {
        // Accounts that live only on other Macs (synced, no card here) count toward the total and get
        // their own legend slice ("Claude · Mac mini") — the number should be the whole truth even
        // when a login isn't set up on this machine.
