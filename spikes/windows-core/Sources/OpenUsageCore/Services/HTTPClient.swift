...
struct URLSessionHTTPClient: HTTPClient {
    ...
    /// One session for every provider request. Proxy support stubbed in the Windows spike.
    private static let session: URLSession = {
        var configuration = URLSessionConfiguration.default
        configuration.applyOpenUsageProxy(ProxyConfig.current)
        if let proxy = ProxyConfig.current {
            AppLog.info(.config, "proxy configured \(proxy.scheme.rawValue)://\(proxy.host):\(proxy.port) (transport stub — not applied)")
        }
    ...
    /// Windows spike: corelibs-foundation has no server-trust override API, so the loopback session
    /// falls back to default validation there (Antigravity is out of the spike's scope anyway).
