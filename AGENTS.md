...
- **Multiple accounts:** a provider that can have several logins on one machine conforms to `MultiAccountProviderRuntime` (`discoverExtraAccounts()` — a cheap local-only scan — plus `makeAccountRuntime(for:)` returning a runtime pinned to one account's credentials). Accounts deliberately change nothing else: metric ids, layout, pins, and enablement stay per-provider; the header's account picker only swaps which login's data fills the card. Core types live in `Sources/OpenUsage/Providers/ProviderAccounts.swift` and `Sources/OpenUsage/Stores/ProviderAccountsStore.swift`; Claude and Codex are the reference implementations.
